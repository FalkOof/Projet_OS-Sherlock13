...
Dans ce projet, l'utilisation des sockets constitue la base de la communication réseau entre le client et le serveur. Le serveur crée une socket TCP à l’aide des appels `socket()`, `bind()`, `listen()` et `accept()`, lui permettant d’écouter les connexions entrantes. De son côté, le client utilise `socket()` et `connect()` pour établir une connexion avec le serveur, et les deux côtés échangent ensuite des messages via `read()` et `write()`.

Le programme repose sur un modèle mono-processus, c’est-à-dire qu’il ne fait pas appel à `fork()` ou à la création de plusieurs processus distincts. Toutes les opérations réseau et logiques sont gérées au sein d’un seul processus par programme. Les pipes, qui sont normalement utilisés pour la communication inter-processus, ne sont donc pas employés ici, car ils ne sont pas nécessaires dans ce contexte réseau.
